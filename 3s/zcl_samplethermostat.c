...
//		HalLedBlink ( HAL_LED_2, 1, 50, 500 );
  ...
  {		
			...
			uint8 add[16];
			UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBR:");
			UART_ZCmdPrintNum 	(HAL_UART_PORT_0, 1);																	
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
			for(i = 0;i<16;i++)
			{
			add[i] = pInReportCmd->attrList[2].attrData[i];
			}
			add[16] == '\0';
			HalUARTWrite		(HAL_UART_PORT_0, add,16); 
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
			UART_ZCmdPrintNum 	(HAL_UART_PORT_0, 2);																	
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
	...
	{	
		...
		uint8 add[16];
		UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBR:");
		UART_ZCmdPrintNum	(HAL_UART_PORT_0,5);
		UART_ZCmdPrintString(HAL_UART_PORT_0, ";"); 
		for(i = 0;i<16;i++)
		{
	...
	if (pInReportCmd->attrList[0].attrID == ATTRID_SENDCURRENT)
		{	
			int i;
			uint32 giatri;
			giatri =  pInReportCmd -> attrList[0].attrData[3];
			giatri = (giatri << 8) + pInReportCmd -> attrList[0].attrData[2];
			giatri = (giatri << 8) + pInReportCmd -> attrList[0].attrData[1];
			giatri = (giatri << 8) + pInReportCmd -> attrList[0].attrData[0];
			
			uint8 add[16];
			UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBR:");
			UART_ZCmdPrintNum	(HAL_UART_PORT_0, 6);
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";"); 
			for(i = 0;i<16;i++)
			{
				add[i] = pInReportCmd->attrList[1].attrData[i];
			}
			add[16] == '\0';
			HalUARTWrite		(HAL_UART_PORT_0, add,16); 
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
        ...
        if (pInReportCmd->attrList[0].attrID == ATTRID_FEEDBACK)
	{
			...
			add[20] == '\0';
			HalUARTWrite		(HAL_UART_PORT_0,add,19); 
			UART_ZCmdPrint		(HAL_UART_PORT_0, "!");
		return;
        }
	// freedata
...
void zclSampleThermostat_CheckAll(void)
{
  ...
  if ( pReportCmd != NULL )
  {
    pReportCmd->numAttr = 1;		
		// Data
    pReportCmd->attrList[0].attrID 		= ATTRID_CHECK_ALL;

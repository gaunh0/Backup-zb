...
char address[] = "00124B000A91644B"; // address of mine
...
static void zclSampleTemperatureSensor_FeedBack(char* data);
...
uint16 zclSampleTemperatureSensor_event_loop( uint8 task_id, uint16 events )
{
  ...
  if ( events & SAMPLETEMPERATURESENSOR_MAIN_SCREEN_EVT )
  {
	...
	{
	  UART_SendString(HAL_UART_PORT_0, "[DEBUG]SW1\n");
...
static void zclSampleTemperatureSensor_HandleKeys( byte shift, byte keys )
{
  ...
  if ( keys & HAL_KEY_SW_7 ) //~ SW3 on board
  {
  	UART_SendString(HAL_UART_PORT_0, "[DEBUG]SW3 \n");
...
void zclSampleTemperatureSensor_FeedBack(char* data)
{
	data[20] = '\0';
#ifdef ZCL_REPORT
...
static void zclSampleTemperatureSensor_SendTemp( void )
{
	...
	data1 = BUILD_UINT16(1,state1);
	data2 = BUILD_UINT16(2,state2);
	
	#ifdef ZCL_REPORT
  zclReportCmd_t *pReportCmd;
	
  pReportCmd = osal_mem_alloc( sizeof(zclReportCmd_t) + 3 * sizeof(zclReport_t) );
  if ( pReportCmd != NULL )
  {
    ...
    pReportCmd->attrList[2].dataType 	= ZCL_DATATYPE_CHAR_STR;
    pReportCmd->attrList[2].attrData 	= (void *)(address);
...
static void zclSampleTemperatureSensor_SendState()
{
	uint8 t_state;
	uint16 state;
	t_state = LED1_SBIT;
  ...
  if ( pReportCmd != NULL )
  {
    ...
    pReportCmd->attrList[0].dataType 	= ZCL_DATATYPE_CHAR_STR;
    pReportCmd->attrList[0].attrData 	= (void *)(address);
  ...
  pInReportCmd = (zclReportCmd_t *)pInMsg->attrCmd;
	...
	if (pInReportCmd->attrList[0].attrID == ATTRID_CONTROL_S)
	{
		...
		//{
			...
			//HalLedSet(HAL_LED_1,HAL_LED_MODE_ON);
		//}
		zclSampleTemperatureSensor_FeedBack(tmp_Data);
		return;
		
	}
	if(pInReportCmd->attrList[0].attrID == ATTRID_CHECK_ALL)
	{	UART_SendString(HAL_UART_PORT_0, "Feedback \n");
		zclSampleTemperatureSensor_SendTemp();
	}
	/*************************** THERMOSTAT DEMAND ******************************/

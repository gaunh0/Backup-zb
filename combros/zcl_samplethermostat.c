...
void zclSampleThermostat_Init( byte task_id )
{
  ...
  zgpTranslationTable_RegisterEP( &zclSampleThermostat_SimpleDesc );
	...
	// sent current state
	osal_start_timerEx(zclSampleThermostat_TaskID,SAMPLETHERMOSTST_CHECK_AFTER_CONNECT,3000);
...
uint16 zclSampleThermostat_event_loop( uint8 task_id, uint16 events )
{
	...
	if ( events & SAMPLETHERMOSTAT_UART_REPLY_CMD_EVT )
  {
	...
	if (events & SAMPLETHERMOSTST_CHECK_AFTER_CONNECT)
	{
		zclSampleThermostat_CheckAll();
	return (events ^ SAMPLETHERMOSTST_CHECK_AFTER_CONNECT);
	}
	/*--------------------------------------------------------------------------*/
...
static void zclSampleThermostat_ProcessInReportCmd( zclIncomingMsg_t *pInMsg )
{
  ...
  {		
			HalLedBlink ( HAL_LED_2, 1, 50, 500 );
			UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBR=1;");					
	...
	{	
			HalLedBlink ( HAL_LED_2, 1, 50, 500 );
			UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBR=5;");					
			UART_ZCmdPrintNum(HAL_UART_PORT_0,pInReportCmd->attrList[0].attrData[0]);
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";");	
			UART_ZCmdPrintNum(HAL_UART_PORT_0,pInReportCmd->attrList[0].attrData[1]);
		    UART_ZCmdPrint		(HAL_UART_PORT_0, "!");
...
if (pInReportCmd->attrList[0].attrID == ATTRID_SENDCURENT)
	{		
			uint32 giatri;
			giatri =  pInReportCmd -> attrList[1].attrData[3];
			giatri = (giatri << 8) + pInReportCmd -> attrList[1].attrData[2];
			giatri = (giatri << 8) + pInReportCmd -> attrList[1].attrData[1];
			giatri = (giatri << 8) + pInReportCmd -> attrList[1].attrData[0];
			
			HalLedBlink ( HAL_LED_2, 1, 50, 500 );
			UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBR=6;");					
			UART_ZCmdPrintNum(HAL_UART_PORT_0,pInReportCmd->attrList[0].attrData[0]); // pos
			UART_ZCmdPrintString(HAL_UART_PORT_0, ";");	
			UART_ZCmdPrintNum(HAL_UART_PORT_0,giatri);
		    UART_ZCmdPrint		(HAL_UART_PORT_0, "!");
		return;
	}
	if ( pInReportCmd->attrList[0].attrID == ATTRID_FEEDBACK)
	{		uint8 i;
			uint8 adds[20];
			UART_ZCmdPrintString(HAL_UART_PORT_0,"@ZBR=2;");
			HalLedBlink ( HAL_LED_2, 1, 50, 500 );
			for(i = 0;i<5;i++)
			{
				adds[i] = pInReportCmd->attrList[0].attrData[i];
			}
			adds[4] == '\0';
			UART_ZCmdPrintString(HAL_UART_PORT_0,adds);
		    UART_ZCmdPrint		(HAL_UART_PORT_0, "!");
			return;
	}
	// freedata
...
void  zclSampleThermostat_SendHenGio(void)
{
	#ifdef ZCL_REPORT
	// Can't send via indirect ??? (system reset)
	afAddrType_t DstAddr;
  ...
  if ( pReportCmd != NULL )
  {
    pReportCmd->numAttr = 2;
		...
		// Data
    pReportCmd->attrList[0].attrID 		= ATTRID_SENDHENGIO;
    pReportCmd->attrList[0].dataType 	= ZCL_DATATYPE_CHAR_STR;
    pReportCmd->attrList[0].attrData 	= (void *)(Free_Data);
		// Endpoint and coordShortAddr
	pReportCmd->attrList[1].attrID 		= NLME_GetCoordShortAddr();
...
void zclSampleThermostat_CheckAll(void)
{
	#ifdef ZCL_REPORT
	// Can't send via indirect ??? (system reset)
	afAddrType_t DstAddr;
  ...
  if ( pReportCmd != NULL )
  {
    pReportCmd->numAttr = 1;
		...
		// Data
    pReportCmd->attrList[0].attrID 		= ATTRID_CHECKALL;

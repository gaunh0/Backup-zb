...
static void zclSampleTemperatureSensor_SendState(void);
static void zclSampleTemperatureSensor_SendFeedback( char* data);
void NV_SaveData(void);
void zclSampleTemperatureSensor_CheckState(void);
...
//  zclSampleTemperatureSensor_DstAddr.addr.shortAddr = NLME_GetCoordShortAddr();
  ...
  HalLcdWriteString( (char *)sDeviceName, HAL_LCD_LINE_3 );
	...
	DHT11_Init();
	// check state
	zclSampleTemperatureSensor_CheckState();
	...
	// set timer for SEnd current
	osal_start_timerEx( zclSampleTemperatureSensor_TaskID, SAMPLETEMPERATURESENSOR_TEMP_SEND_EVT, 5000 );
	//osal_set_event(zclSampleTemperatureSensor_TaskID,SAMPLETEMPERATURESENSOR_SENDCURRENT);
 ...
 ****************************************/
  ...
  if ( events & SAMPLETEMPERATURESENSOR_TEMP_SEND_EVT )
  {
	...
	osal_start_timerEx(zclSampleTemperatureSensor_TaskID,SAMPLETEMPERATURESENSOR_TEMP_SEND_EVT,60000);
    return ( events ^ SAMPLETEMPERATURESENSOR_TEMP_SEND_EVT );
  }
  if (events & SAMPLETEMPERATURESENSOR_SENDCURRENT)
  	{
  		zclSampleTemperatureSensor_Current();
		osal_start_timerEx(zclSampleTemperatureSensor_TaskID,SAMPLETEMPERATURESENSOR_SENDCURRENT,20000);
		return ( events ^ SAMPLETEMPERATURESENSOR_SENDCURRENT );
  	}
	/*--------------------------------------------------------------------------*/
...
static void zclSampleTemperatureSensor_HandleKeys( byte shift, byte keys )
{
  ...
  {
  	UART_DebugPrint(HAL_UART_PORT_0, "SW7");
...
//----------------------------
void zclSampleTemperatureSensor_Current( void )
{

	uint32 data;
	data = rand()%100;
	uint16 state;
	state = BUILD_UINT16(pos,0);
	#ifdef ZCL_REPORT
  zclReportCmd_t *pReportCmd;
  ...
  if ( pReportCmd != NULL )
  {
    pReportCmd->numAttr = 2;
		// Data
    pReportCmd->attrList[0].attrID = ATTRID_SENDCURENT;
    pReportCmd->attrList[0].dataType = ZCL_DATATYPE_INT16;
    pReportCmd->attrList[0].attrData = (void *)(&state);

    pReportCmd->attrList[1].dataType = ZCL_DATATYPE_INT32;
    pReportCmd->attrList[1].attrData = (void *)(&data);
...
static void zclSampleTemperatureSensor_SendFeedback( char* data )
{
	data[4] = '\0';
	  ...
	  if ( pReportCmd != NULL )
	  {
		pReportCmd->numAttr = 1;
			// Data
		pReportCmd->attrList[0].attrID = ATTRID_FEEDBACK;
...
//		HalLedSet(HAL_LED_4,HAL_LED_MODE_BLINK);
	...
	if (pInReportCmd->attrList[0].attrID == ATTRID_CHECKALL)
	{
//	HalLedSet(HAL_LED_4,HAL_LED_MODE_BLINK);
		zclSampleTemperatureSensor_SendTemp();
	}
	if (pInReportCmd->attrList[0].attrID == ATTRID_SENDHENGIO)
		{
...
//			HalLedSet(HAL_LED_4,HAL_LED_MODE_BLINK);
			...
			if (strstr(tmp_Data,"31;1"))
			{
				HalLedSet(HAL_LED_2,HAL_LED_MODE_ON);
				HalLedSet(HAL_LED_3,HAL_LED_MODE_BLINK);
			}
			else if (strstr(tmp_Data,"31;0"))
				{
			  HalLedSet(HAL_LED_2,HAL_LED_MODE_OFF);
			  HalLedSet(HAL_LED_3,HAL_LED_MODE_BLINK);
			}
...
//		HalLedSet(HAL_LED_4,HAL_LED_MODE_BLINK);
		...
		{
		NV_SaveData();
		zclSampleTemperatureSensor_SendFeedback(tmp_Data);
		}
		return;
...
void NV_SaveData(void)
{
	uint8 state;
	state = LED1_SBIT;
	if(osal_nv_item_init(ZCD_NV_SAVE_DATA,1,(void*)&state)== SUCCESS)
	{
	osal_nv_write(ZCD_NV_SAVE_DATA,0x0,1,(void*)&state);
	}
}
void zclSampleTemperatureSensor_CheckState(void)
{
 	uint8 data;
	if(osal_nv_item_init(ZCD_NV_SAVE_DATA,1,(void*)&data)== SUCCESS)
		{
		osal_nv_read(ZCD_NV_SAVE_DATA,0x0,1,(void*)&data);
	}
	if (data == 0)
	{
		HalLedSet(HAL_LED_1,HAL_LED_MODE_OFF);	
